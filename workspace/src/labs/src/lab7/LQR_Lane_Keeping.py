...
class image_processing_node():
    ...
    def draw_lines(self,img, edges, color=[0, 0,255 ], thickness=3):
        ...
        for y_base in xrange(y_newPixel_min,y_newPixel_max,interval):
            ...
            if (not(y_base==y_newPixel_min) and (x_right-x_left)<converge_limit):
                ...
                #print 'the car should have topped'
            if self.stopmoving:
                x_left = leftpts[-1][0]
                x_right = rightpts[-1][0]
                y_left = leftpts[-1][1]
    ...
    def calc_x_newPixel_to_y_Inertial(self,x_newPixel,y_newPixel):
        ...
        y_Inertial = -0.0067*x_newPixel + 0.0078#FILL IN HERE
    ...
    def calc_y_newPixel_to_x_Inertial(self,y_newPixel):
        ...
        x_Inertial = 0.0003*y_newPixel**2 - 0.0523*y_newPixel + 3.8228#FILL IN HERE
    ...
    def compute_uOpt(self,x_ref,y_ref,v_ref):
        try:
            if not(self.stopmoving):
                ...
                Ac = np.matrix([[0.0, 0.0, -v_ref*sin(psi_des + beta_des)],
                    [0.0, 0.0, v_ref*cos(psi_des + beta_des)],
                    [0.0, 0.0, 0.0]])
                    # TO DO
                Bc = np.matrix([[cos(psi_des + beta_des), -v_ref*sin(psi_des + beta_des)],
                    [sin(psi_des + beta_des), v_ref*cos(psi_des + beta_des)],
                    [sin(beta_des)/lr, v_ref/lr*cos(beta_des)]])
                    # TO DO

                Q = np.matrix([[1, 0, 0], [0, 10, 0], [0, 0, 1]])# TO DO
                R = np.matrix([[2, 0], [0, 2]])# TO DO

                # Compute the LQR controller
                K, X, closedLoopEigVals = controlpy.synthesis.controller_lqr(Ac, Bc, Q, R)# TO DO
                ...
                #print('desired velocity : {0}'.format(u_Opt[0,0]))
                #print('desired steering angle : {0}'.format(u_Opt[1,0]*180/pi))
                #print('')
                # print(K)
